...
export default class extends Controller {
  ...
  addActValues(event) {
    ...
    row.innerHTML = `
      <div class="flex md:flex-row flex-col md:items-center md:gap-2 md:my-2 md:text-xl text-sm gap-0.5 my-0.5">
  ...
  addBolus(event) {
    event.preventDefault()
    const row = document.createElement("div")
    row.classList.add("bolus-row", "md:flex", "items-center")
    row.innerHTML = `
      <div class="flex items-center md:gap-2 md:my-2 md:text-xl text-sm gap-0.5 my-0.5">
        ...
        <input placeholder="Total UI per shot" type="number" name="[heparins][bolus_given]" id="_heparins_bolus_given" min="1" class="md:flex-1 flex-shrink-1 rounded-lg border-2 border-quaternary bg-secondary-light text-secondary-dark md:p-2 p-1 focus:border-icon-color-dark data-hj-allow md:text-base text-xs">

        <label for="_heparins_bolus_time" class="md:text-xl text-sm flex items-center md:mr-2 mr-1">Time</label>
        <input type="datetime-local" name="[heparins][bolus_time]" id="_heparins_bolus_time" class="md:flex-1 flex-shrink-1 rounded-lg border-2 border-quaternary bg-secondary-light text-secondary-dark md:p-2 p-1 focus:border-icon-color-dark data-hj-allow md:text-base text-xs">
      </div>
      <div class="justify-center items-center md:flex">
